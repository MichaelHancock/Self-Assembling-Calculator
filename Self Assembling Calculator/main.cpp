...
std::vector<std::string> composeFunctions(std::string, int);
...
int main() {
	...
	for (int i = 2; i < 7; i++) {
		...
		std::cout << "Generating functions of length: " << i << std::endl;
		std::vector<std::string> functions = composeFunctions(requires, i);
...
std::vector<std::string> composeFunctions(std::string requiredInterfaces, int solutionLength) {
	...
	std::vector<std::string> result = functionGenerator.generateAllFunctonInstances(solutionLength);
